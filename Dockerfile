...
RUN echo "🚀 Starting RootFS setup process..." && \
    ...
    echo "📥 Starting FEXRootFSFetcher.." && \
    ...
    \
    # FALLBACK: Manual setup using predownloaded RootFS from build context
    if [ "$FEXROOTFS_SUCCESS" = "false" ]; then \
        echo "🔄 FEXRootFSFetcher failed - falling back to manual setup with predownloaded files..." && \
        \
        # Create necessary directories for manual setup
        mkdir -p /home/fex/.fex-emu/RootFS && \
        \
        # Find RootFS file in build context 
        echo "🔍 Looking for RootFS files in build context..." && \
        ls -la /tmp/fex-rootfs/ && \
        \
        # Detect RootFS file type and name
        ROOTFS_FILE="" && \
        ...
        if [ -z "$ROOTFS_FILE" ]; then \
            echo "❌ No RootFS file found in build context" && \
            exit 1; \
        fi && \
        \
        ...
        echo "📦 Extracting RootFS from build context..." && \
